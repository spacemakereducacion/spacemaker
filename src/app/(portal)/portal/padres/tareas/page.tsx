...
export default async function GuardianTasksPage({
  searchParams,
}: {
  ...
  return (
    <>
      ...
      <ChildSwitcher students={students} currentId={student?.id} basePath="/portal/padres/tareas" />
      <ul className="space-y-2">
        {student?.assignmentSubmissions.map((item) => (
          <li key={item.id} className="rounded-xl border border-border bg-card px-4 py-3">
            {item.assignment.title} · {item.score?.toString() ?? "enviada"}
          </li>
        ))}
      </ul>
    </>
